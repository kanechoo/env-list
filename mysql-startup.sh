...
if [ -f "mysql-startup.env" ]; then
  . mysql-startup.env
else
  echo "Cannot find the env file [mysql-startup.env] in current folder"
fi
...
if docker ps --format '{{.Image}}' | grep "$IMAGE:$TAG" >/dev/null 2>&1; then
  ...
  if docker container ls -a --format '{{.Image}}' | grep "$IMAGE:$TAG" >/dev/null 2>&1; then
    ...
    docker run --name "$CONTAINER" --network "$NETWORK" --env-file="$ENV_FILE" -v "$VOLUME":/var/lib/mysql -p "$PORT":3306 --bind-address=0.0.0.0 --skip-name-resolve -d --restart always "$IMAGE:$TAG"
  fi
fi
